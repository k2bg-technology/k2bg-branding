...
describe('generateMetadata', () => {
  ...
  it.each(
    supportedLanguages
  )('sets the canonical URL to the localized page URL for "%s"', async (language) => {
    vi.stubEnv('PORTFOLIO_SITE_BASE_URL', configuredBaseUrl);
  ...
  it.each(
    supportedLanguages
  )('lists hreflang alternates for every supported language on "%s"', async (language) => {
    vi.stubEnv('PORTFOLIO_SITE_BASE_URL', configuredBaseUrl);
  ...
  ])('sets the Open Graph URL and locale for "$language"', async ({
    language,
    locale,
  }) => {
    vi.stubEnv('PORTFOLIO_SITE_BASE_URL', configuredBaseUrl);
  ...
  ])('uses the localized description for "$language"', async ({
    language,
    description,
  }) => {
    vi.stubEnv('PORTFOLIO_SITE_BASE_URL', configuredBaseUrl);
