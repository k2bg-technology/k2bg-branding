...
- Semantic chart thresholds, gaps, keyboard interaction, and deterministic calculations are behavior even when their results affect appearance (`packages/ui/src/components/Chart/BarChart.test.tsx`, `apps/scene-studio/src/patterns/VisualShowcase/kenBurns.test.ts`)
- An accessible role or name, an attribute that changes what assistive technology announces or how the keyboard reaches an element (`aria-live`, `aria-expanded`, `tabIndex`), a link target, whether an element renders under a condition, and an attribute a consumer relies on (a forwarded class name, `colSpan`, `data-*` state) are behavior, asserted with role and accessible-name queries (`packages/ui/src/components/Table/Table.test.tsx`)
...
- A declarative schema passing a valid value through unchanged (an explicit enum value, an optional string, a `strictObject` field); that tests the library. A schema is covered by rejection of invalid values at the trust boundary, defaults, custom refinements and transforms, and one representative valid fixture that loads and uses every configuration value the app promises (`apps/observatory/modules/dashboard/adapters/output/definition-sources/file-system/schemas/statTiles.ts` — every reduction)
...
- Assert outgoing interactions only when the call is the requirement: an externally observable effect (sending email, uploading an image, revalidating a path), a documented request contract to an external API or driver (a Notion filter, Cloudinary upload options, postgres pool options), an ordering or guard rule (rate limit before captcha in `apps/blog/app/contact/contactFormAction.test.ts`, validation before any side effect), or a single-flight rule whose violation changes cost or security (one connection per Lambda, one loader call per cache key). A call count that only proves efficiency is not asserted. Prefer an input-keyed fake whose output proves the call, as in `apps/observatory/modules/dashboard/adapters/output/query-services/warehouse/queryServices.test.ts`: it fails when the request is wrong without listing arguments
...
- Retain logging assertions for an operational output contract: the request log fields stated in `.claude/rules/hono-api-guidelines.md`, error-level logging of failures, the warn/error discrimination in `.claude/rules/error-handling-guidelines.md`, and redaction in emitted output (`packages/logger/index.test.ts`). Assert the level and the structured fields, not the message text, unless the text is the contract
...
2. Delete a test when another test asserts the same input class with the same expected result through the same expression at the same boundary. A composition test (`apps/blog/server/app.test.ts`) and a unit test of one part (`apps/blog/server/middleware/apiKeyAuth.test.ts`) are different guarantees and both stay. A per-value acceptance test of a promised configuration value is a copy only when a representative valid fixture that uses every promised value exists; until then it stays.
3. Move a test that asserts only decorative appearance to a story (`packages/ui`, `apps/blog`, `apps/portfolio`) or to a Scene Studio demo composition. The unit test is deleted once a story or demo shows that state; Chromatic and the demo compositions are the verification, and a blocking CI check is not required. When no story or demo shows the state, the same pull request adds one and then deletes the unit test. In a package with no story or demo layer (`apps/observatory`), the decorative unit test stays.
